...
namespace fabricad::blocks {
  ...
  void ArcLintel::render()
  {
    ...
    float outer_length = ellipseCircumference(a_ + arcLength(), b_ + arcLenght()) / 4;
    ...
    float step = ((M_PI / 2) - start_angle) / nr_of_rays;
  ...
  layers_[0].polygons.push_back(outer);
